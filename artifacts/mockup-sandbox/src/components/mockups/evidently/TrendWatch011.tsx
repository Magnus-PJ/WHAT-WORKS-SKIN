// TrendWatch011 — Issue 011 archive: Bakuchiol vs retinol, the Seoul split-face.
...
const VERDICTS = [
  { n: "01", name: "Bakuchiol 0.5% nightly", tier: "B" as const, verdict: "Holds Up", color: T.tierB,
    body: "The new 12-week split-face trial out of Seoul (n=84) found bakuchiol 0.5% produced a measurable, if modest, improvement in fine lines and pigmentation when applied nightly. It does not match a 0.025% tretinoin in head-to-head endpoints, but it does so with a fraction of the irritation. For people who genuinely cannot tolerate retinoids, it is the most defensible alternative on the shelf.",
    bottom: "Real molecule, modest endpoints. Use as a tolerance bridge — not as a like-for-like retinoid." },
  { n: "02", name: "Bakuchiol as 'pregnancy-safe retinol'", tier: "D" as const, verdict: "Misleading", color: T.tierD,
    ...
    bottom: "Safe in pregnancy. Not equivalent to a retinoid. Stop conflating the two." },
];

const TrendWatch011: React.FC = () => (
  <div className="relative" style={{ background: T.paper, color: T.ink, fontFamily: SANS }}>
    <LaidPaper />
    <SiteHeader active="Trend Watch" />
    <Breadcrumbs trail={[{ label: "Home", href: "#" }, { label: "Trend Watch", href: "#" }, { label: "Issue 011 · Archive" }]} />
        ...
        <div className="relative grid grid-cols-12 gap-10 py-24">
          <div className="col-span-12 lg:col-span-9 lg:pl-12">
            <div className="flex items-center justify-between border-b pb-4" style={{ borderColor: T.rule }}>
              <Eyebrow color={T.accent}>Trend Watch · Issue 011 · 09 March 2026</Eyebrow>
            ...
            <div className="mt-10 inline-flex items-center gap-3 px-4 py-2 border" style={{ borderColor: T.rule, background: T.paper2 }}>
              <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: "0.18em", color: T.muted, textTransform: "uppercase" }}>Issue № 011 · Archive</span>
            </div>
            <h1 className="mt-6" style={{ fontFamily: SERIF, fontSize: 110, lineHeight: 0.92, letterSpacing: "-0.045em", fontWeight: 400, color: T.ink, fontVariationSettings: '"opsz" 144, "SOFT" 30' }}>
              Bakuchiol, redux. <br />
              <span style={{ fontStyle: "italic", color: T.accent, fontFamily: SERIF_ED }}>The Seoul split-face data.</span>
            </h1>
            <p className="mt-9 max-w-2xl" style={{ fontFamily: SERIF, fontSize: 21, lineHeight: 1.55, color: T.inkSoft }}>
              An 84-participant trial dropped that should settle the debate, and instead, sharpened it. The molecule is real. The marketing claim that wraps it is not.
            </p>
          </div>
        </div>
      </Container>
    ...
    <TrendWatchRelated component="TrendWatch011" />

    <SiteFooter />
  </div>
);

export default TrendWatch011;
