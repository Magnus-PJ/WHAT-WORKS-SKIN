// ConcernHormonalAcne — Adult / hormonal acne guide.
...
const ROW = concernRowFor("hormonal-acne");

const SIGNS = [
  { k: "Jawline / chin pattern", w: "85%", n: "Distinguishes from comedonal teen acne, which favours the T-zone." },
  { k: "Premenstrual flares",     w: "78%", n: "Days 18–28 of the cycle. Predictable, hormonal." },
  { k: "Persists past 25",         w: "70%", n: "Adult-onset or persistent — the defining feature." },
  { k: "Co-existing PCOS signs",  w: "32%", n: "Hirsutism, irregular cycles. Worth a workup if present." },
];

const INGREDIENTS = [
  { name: "Adapalene 0.1%",        tier: "A" as const, role: "Comedolytic retinoid", evidence: "Reference OTC retinoid · 12-week RCTs" },
  { name: "Tretinoin 0.025–0.05% (Rx)", tier: "A" as const, role: "Retinoid", evidence: "Stronger; pregnancy-contraindicated" },
  { name: "Benzoyl peroxide 2.5%", tier: "A" as const, role: "Antibacterial",     evidence: "Reference for inflammatory papules · over decades" },
  { name: "Azelaic acid 15–20% (Rx)", tier: "A" as const, role: "Anti-inflammatory · pigment", evidence: "Excellent in pregnancy & PIH-prone skin" },
  { name: "Spironolactone (Rx, oral)", tier: "A" as const, role: "Anti-androgen", evidence: "Strong in adult women · derm-prescribed" },
  { name: "Salicylic acid 2%",     tier: "B" as const, role: "BHA",                evidence: "Mild adjunct · not a primary treatment" },
];

const PROTOCOL_AM = [
  "Low-pH gel cleanser",
  "Niacinamide 5% — calming",
  "Lightweight moisturiser (CeraVe AM, Toleriane Double Repair)",
  "Mineral or hybrid SPF 50+ — non-comedogenic",
];
const PROTOCOL_PM = [
  "Cleansing oil → gel cleanser (double-cleanse if wearing SPF + makeup)",
  "Adapalene 0.1% (or tretinoin) — pea-sized, full face",
  "Buffer: ceramide moisturiser",
  "Spot-treatment: BPO 2.5% on inflammatory lesions only",
];

const ConcernHormonalAcne: React.FC = () => (
  <div className="relative" style={{ background: T.paper, color: T.ink, fontFamily: SANS }}>
    <LaidPaper />
    <SiteHeader active="Concerns" />
    <Breadcrumbs trail={[{ label: "Home", href: "#" }, { label: "Concerns", href: "#" }, { label: "Hormonal acne" }]} />
        ...
        <div className="relative grid grid-cols-12 gap-10 py-24">
          <div className="col-span-12 lg:col-span-8 lg:pl-12">
            <div className="flex items-center justify-between border-b pb-4" style={{ borderColor: T.rule }}>
              ...
              <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.12em", color: T.mutedSoft }}>P. 14 · GUIDE</div>
            </div>
            ...
            <h1 className="mt-10" style={{ fontFamily: SERIF, fontSize: 108, lineHeight: 0.93, letterSpacing: "-0.045em", fontWeight: 400, color: T.ink, fontVariationSettings: '"opsz" 144, "SOFT" 30' }}>
              Hormonal <br />
              <span style={{ fontStyle: "italic", color: T.accent, fontFamily: SERIF_ED }}>acne.</span>
            </h1>
          ...
          <aside className="col-span-12 lg:col-span-4 lg:flex lg:items-end">
            <div className="border" style={{ borderColor: T.rule, background: T.paper }}>
              ...
              <dl className="divide-y" style={{ borderColor: T.rule }}>
                {[
                  ["Prevalence", "~25% women 25–40 in India"],
                  ["Pattern",     "Jawline & chin · cyclical · adult-onset"],
                  ["Driver",      "Androgenic stimulation of sebocyte"],
                  ["Outlook",     "Excellent on topical + (sometimes) systemic"],
    ...
    <section className="relative z-10 border-b py-20" style={{ borderColor: T.rule }}>
      <Container>
        ...
        <h2 className="mt-2" style={{ fontFamily: SERIF, fontSize: 56, lineHeight: 1, fontWeight: 400, fontVariationSettings: '"opsz" 144, "SOFT" 30', color: T.ink, letterSpacing: "-0.03em" }}>
          Recognising the <span style={{ fontStyle: "italic", color: T.accent, fontFamily: SERIF_ED }}>pattern.</span>
        </h2>
    ...
    <section className="relative z-10 border-b py-20" style={{ borderColor: T.rule, background: T.paper2 }}>
      <Container>
        <Folio>§ 02</Folio>
        <h2 className="mt-2" style={{ fontFamily: SERIF, fontSize: 56, lineHeight: 1, fontWeight: 400, fontVariationSettings: '"opsz" 144, "SOFT" 30', color: T.ink, letterSpacing: "-0.03em" }}>
          Ingredients that <span style={{ fontStyle: "italic", color: T.accent, fontFamily: SERIF_ED }}>work.</span>
        </h2>
    ...
    <section className="relative z-10 border-b py-20" style={{ borderColor: T.rule }}>
      <Container>
        ...
        <p className="mt-7 max-w-3xl" style={{ fontFamily: SERIF, fontSize: 17, color: T.inkSoft, lineHeight: 1.6 }}>
          If this stack does not produce visible improvement at 12 weeks, the third lever is systemic — a derm consult about spironolactone, COCPs, or oral isotretinoin (in severe cases). Topicals alone resolve the majority of adult-acne presentations, but not all of them.
        </p>
      </Container>
    </section>

    <EditorPageLink pageKind="concern" pageSlug="ConcernHormonalAcne" />
    <SiteFooter />
  </div>
);

export default ConcernHormonalAcne;
